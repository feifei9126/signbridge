...
<img width="3516" height="1834" alt="ScreenShot_2026-07-21_102027_932_副本" src="https://github.com/user-attachments/assets/c1c4a27a-8cdf-49e6-8170-43264f2b8b76" />
...
1. **实时字幕→手语动画**：从 B站、YouTube、Netflix 等网页播放器捕获可用字幕，低延迟播放已收录动作
...
字幕/语音文本
  ...
  ▼
translateText()
  ├─ 从左到右最长匹配          → 52 条结构化词条（gloss + meta + tags）
  ├─ 单字回退                  → 16 个高频单字手势
  ├─ 未知内容跳过              → 不用错误动作冒充翻译
  └─ 多个命中动作按原顺序合并  → 一句字幕可连续播放多个已知词
  │
  ▼
playFrames(frames)
...
npm test            # 构建并运行词典/构建产物测试
npm run verify      # 构建、Lint、测试和格式检查
```
...
当前版本不是通用 AI 手语翻译器。它会把字幕切分为词典中已收录的 gloss，再播放对应骨骼关键帧；未知词会跳过。因此，词典覆盖率、动作准确性和句法重排能力决定最终效果。

后续最稳妥的方案不是继续在运行时代码里反复猜骨骼轴，而是拆成三个可独立验证的层：

1. **文本层**：字幕/语音统一进入规范化与 CSL gloss 转换，输出有序 gloss 列表。
2. **动作资产层**：每个 gloss 关联经过人工审核的关键帧或 glTF AnimationClip，并记录手形、位置、朝向、运动和非手控特征。
3. **模型适配层**：为每个虚拟人保存一次骨骼映射、REST 姿态和坐标系重定向配置，动作数据不再跟具体模型骨骼名耦合。

近期应优先扩充并验收标准动作资产；中期加入独立的模型重定向器和动作回归截图；远期再引入句法模型。直接由字幕或视频生成任意关节角度难以审核，也最容易再次出现手掌反向、穿模和模型更换后失效。
